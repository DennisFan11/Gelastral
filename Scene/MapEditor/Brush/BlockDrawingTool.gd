...
func _L_click(): # exec-once
	_hint_polygon.color = _color_map[COLOR.BUILD]
...
func _L_clicking():
	...
	if Type == TYPE.POINT:
			...
			MapManager.merge(_gen_points(_second_point), ID)

func _L_finish(): #exec-once
	if Type == TYPE.LINE:
		#if not MapManager.merge_busy:
			MapManager.merge(_gen_line_points(_first_point, _second_point), ID)


func _Idle(): # 未按下
	_hint_polygon.color = _color_map[COLOR.IDLE]
	_first_point = get_global_mouse_position()
	_second_point = get_global_mouse_position()
	update_hint_polygon()
...
func _R_click(): # exec-once
	_hint_polygon.color = _color_map[COLOR.DELETE]
	_first_point = get_global_mouse_position()
...
func _R_clicking():
	...
	update_hint_polygon()
	if Type == TYPE.POINT:
		MapManager.clip(_gen_points(_second_point))

func _R_finish(): #exec-once
	if Type == TYPE.LINE:
		MapManager.clip(_gen_line_points(_first_point, _second_point))
...
func _exit_tree() -> void:
	_hint_polygon.queue_free()


func update_hint_polygon():
	match Type:
		TYPE.LINE:
			_hint_polygon.polygon = _gen_line_points(_first_point, _second_point)
		TYPE.POINT:
			_hint_polygon.polygon = _gen_points(_second_point)
